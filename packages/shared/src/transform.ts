...
export async function transform(
	...
	switch (mode) {
		case 'perFile':
			return await perFileTransform(code, elements);
		case 'all':
			if (elements === undefined) {
				throw new Error("elements argument is required in 'all' mode.");
			}
			return await allTransformation(code, elements);
	}
}
...
export async function allTransformation(code: string, elements: string[]) {
